     ...
     flex flex-col items-start space-y-2 shadow-md hover:shadow-lg p-4 rounded-md w-auto mx-2 h-72  sm:w-64`}>
        ...
        <div className='flex flex-col justify-center items-center space-y-2 w-full text-xs text-left
         text-gray-500  font-semibold'>
           <Link to={`channel/${video?.snippet?.channelId}`} className='hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'>
            { video?.snippet?.channelTitle?.slice(0,20) } { video?.snippet?.channelTitle.length > 20 && ' ...' }</Link>
